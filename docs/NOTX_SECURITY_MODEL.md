...
```
# notx/1.0
# note_urn:      urn:notx:note:01HZX3K8SECURENOTE123456
# note_type:     secure
...
- A `DeviceIdentity` struct containing:
  - `DeviceID` — a `urn:notx:device:<id>` URN (new URN object type)
...
| -------- | ------------------------ | ------------------------------------------ |
| `device` | A registered user device | `urn:notx:device:01HZX3K8DEVICEID12345678` |
...
- [ ] `DeviceID` follows the `urn:notx:device:<id>` URN format and validates correctly
...
key[urn:notx:device:01HZX3K8DEVICE2ID1234567]: <base64-wrapped-cek>
```
